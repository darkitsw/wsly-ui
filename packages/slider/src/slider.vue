<template>
  <div class="ws-slider">
    <div ref="wrap" :class="['ws-slider-wrap', disabledClass, { 'ws-slider-has-label': piecewiseLabel }]" v-show="show" :style="wrapStyles" @click="wrapClick">
      <div ref="elem" class="ws-slider" :style="[elemStyles, bgStyle]">
        <template>
          <div ref="dot" :class="[tooltipStatus, 'ws-slider-dot']" :style="[sliderStyles, dotStyles]" @mousedown="moveStart">
            <span :class="['ws-slider-tooltip-' + tooltipDirection, 'ws-slider-tooltip']" :style="tooltipStyles">{{ formatter ? formatting(val) : val }}</span>
          </div>
        </template>
        <div ref="process" class="ws-slider-process" :style="processStyle"></div>
      </div>
    </div>
  </div>
</template>
<style lang="scss">
  ...
  .ws-slider-tooltip-top{
    ...
    .ws-slider-tooltip-right::before  {
      content: '';
      position: absolute;
      top: 50%;
      left: -10px;
  ...
  .ws-slider-dot.ws-slider-hover:hover .ws-slider-tooltip {
    display: block;
  }
...
<script>
  export default {
    ...
    computed: {
      ...
      tooltipStatus () {
        return this.tooltip === 'hover' && this.flag ? 'ws-slider-always' : this.tooltip ? `ws-slider-${this.tooltip}` : ''
      },
      isDisabled () {
        return this.eventType === 'none' ? true : this.disabled
      },
      disabledClass () {
        return this.disabled ? 'ws-slider-disabled' : ''
      },
      isRange () {
